...
st.markdown('<div class="main-header">🧠 Bayesian SVI (Stochastic Variational Inference) Engine</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-header">Full Bayesian factor model | ARD priors | SVI with AutoNormal guide | Factor loading magnitude = score | Multi‑window evaluation</div>', unsafe_allow_html=True)
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'svi_bayesian_' in f]
    if not json_files:
        return None
...
st.caption("Full Bayesian factor model with SVI (Pyro). Higher loading → stronger factor exposure → overweight signal.")
